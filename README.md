...
Audit date: 2026-08-31. Updated 2026-09-01 with the standalone 1024 px MitUNet run and 2026-09-02 with the AdamW optimizer comparator.

## Scope

The current audit covers seven experiments:
...
Start at [docs/visual_comparisons/shared_validation_samples.md](docs/visual_comparisons/shared_validation_samples.md) for the shared-sample index, [docs/visual_qa_generation_summary.json](docs/visual_qa_generation_summary.json) for the core QA generation summary, and [docs/visual_comparisons/adamw_optimizer_samples.md](docs/visual_comparisons/adamw_optimizer_samples.md) for the AdamW comparator visuals. Overlay colors are green for true positive, red for false positive, and blue for false negative where generated by the audit scripts.
...
The exact baseline source git commit and DVC hash were not located in the inspected baseline artifacts. The standalone 1024 px run records a source git commit, but its captured git state was dirty. The AdamW harness directory is not a Git repository, though the protected core implementation is clean and commit-pinned. Several Phase 1 historical values mentioned in task context were not found in the Phase 1 run reports and are therefore not recorded as verified. Phase 1 and Phase 2 checkpoint SHA-256 hashes are deferred in their manifests, so this repository records checkpoint paths and sizes but not hashes.
