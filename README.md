...
Experiment 10 is the repository's first target-version (label) ablation, and its primary question is label correctness rather than model performance. Two MitUNet arms were trained from one byte-identical initialization on identical plans in identical order, differing only in the door-opening target version: `candidate_v3.1` (control) against `candidate_v3.2` (treatment). **On the label question, v3.2 repaired what it targeted.** A reviewer accepted 5 of the 7 instances adjudicated so far, including both instances where `candidate_v3.1` had been marked a `Critical issue`; the Family A geometry audit finds all 10 repaired instances tangent-aligned with none placed on the wrong wall; one instance that produced no opening at all under v3.1 now produces a correct jamb-to-jamb opening; and no door-opening warnings remain in scope. That is not a corpus-level claim: it covers 10 instances of 45,857, the re-review queue is 7 of 44 rows (15.9%) with all 34 regression controls unreviewed, one of v3.1's three confirmed failures is still unresolved, and the 49-row gold queue is 0 resolved. A defect in the review app's Skip button had destroyed four of those five `Correct` verdicts; the loss was detected from the app's own audit log, the tool was fixed and the verdicts restored on 2026-09-25.
