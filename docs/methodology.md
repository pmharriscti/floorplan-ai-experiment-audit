...
- `negative_label_version_ablation`: a completed target-version (label) ablation in which the treatment label version did not improve on the control. Pixel-level differences may be statistically indistinguishable while an object-level metric significantly favours the control. The label is deliberately weak on causation: when the two versions differ on only a handful of training masks and the evaluation plans have identical targets, the measured difference separates the two trained models rather than the two label versions, and the entry says so.
